...
========================================================================================
                         nf-core/msspe
========================================================================================
 nf-core/msspe Analysis Pipeline.
 #### Homepage / Documentation
 https://github.com/nf-core/msspe
----------------------------------------------------------------------------------------
*/
...
def helpMessage() {
    ...
    Mandatory arguments:
      ...
      --reads [file]                Path to input data (must be surrounded with quotes)
			--primers [file]              Path to BED file of primers
      --fasta [file]                Path to FASTA reference sequence
...
Channel.from(summary.collect{ [it.key, it.value] })
    ...
    .map { x -> """
    id: 'nf-core-msspe-summary'
    description: " - this information is collected when the pipeline is started."
    section_name: 'nf-core/msspe Workflow Summary'
...
process makeConsensus {
	...
	samtools mpileup -A -d ${params.mpileupDepth} -Q0 ${bam} \\
		| ivar consensus -q ${params.ivarQualThreshold} \\
				-t ${params.ivarFreqThreshold} \\
				-m ${params.ivarMinDepth} \\
				-n N \\
				-p ${sampleName}.primertrimmed.consensus
	"""
}
...
workflow.onComplete {
    ...
    def subject = "[nf-core/msspe] Successful: $workflow.runName"
    if (!workflow.success) {
        subject = "[nf-core/msspe] FAILED: $workflow.runName"
    }
    def email_fields = [:]
    ...
    try {
        if (workflow.success) {
            ...
            if (mqc_report.getClass() == ArrayList) {
                log.warn "[nf-core/msspe] Found multiple reports from process 'multiqc', will use only one"
                mqc_report = mqc_report[0]
            }
        }
    } catch (all) {
        log.warn "[nf-core/msspe] Could not attach MultiQC report to summary email"
    }
    ...
    if (email_address) {
        try {
            ...
            log.info "[nf-core/msspe] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
            // Catch failures and try with plaintext
            [ 'mail', '-s', subject, email_address ].execute() << email_txt
            log.info "[nf-core/msspe] Sent summary e-mail to $email_address (mail)"
        }
    }
    ...
    if (workflow.success) {
        log.info "-${c_purple}[nf-core/msspe]${c_green} Pipeline completed successfully${c_reset}-"
    } else {
        checkHostname()
        log.info "-${c_purple}[nf-core/msspe]${c_red} Pipeline completed with errors${c_reset}-"
    }
...
def nfcoreHeader() {
    ...
    ${c_blue}  | \\| |       \\__, \\__/ |  \\ |___     ${c_green}\\`-._,-`-,${c_reset}
                                            ${c_green}`._,._,\'${c_reset}
    ${c_purple}  nf-core/msspe v${workflow.manifest.version}${c_reset}
    -${c_dim}--------------------------------------------------${c_reset}-
    """.stripIndent()
}
